...
bGRPNAMEappend = 0 -- some globals we will need for naming groups
rGRPNAMEappend = 0
...
for element = 1, #tblCheck do -- for each element in the table from 1 to the amount that is in the table
	local checkThisGroup = tblCheck[element] -- get the string of the group name located at that index
	local msg = {}
	local initsize = tblCheck[grpTYPE][sideUsed].INITSIZE[element]
	local currentsize = tblCheck[grpTYPE][sideUsed].SIZE[element]
...
function moveStuff(thisGroupName, randomizerDist, spd, team)	
	...
	if team == "RED" then
		...
		local randomizer_dir = mist.random(0,60)
		end
		if negativeheading == 1 then
		local randomizer_dir = mist.random(300,360)
		end		
	end
	...
	local randomizer_dir = mist.random(120,240)
	end
...
function ColorZones(routine, coalitionSide)
